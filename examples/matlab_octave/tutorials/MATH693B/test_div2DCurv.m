...
% This is an example of a curvilinear grid setup and 
% using mimetics to solve on the grid
%
% This harnesses the "Family" aspect of the operators.
% Each operator is made of smaller operators you the user
% can use however you want, knowing together they are mimetic.
%
%
clc
close all
...
%% Interpolators - showing off how to move things
% Some useful numbers
num_nodes = (m+1)*(n+1);
num_centers = (m+2)*(n+2);
num_u = (m+1)*n;
...
% Interpolate U values (initial conditions)
Ugiven = sin(X);
U = NtoU * Ugiven(:);
U = reshape(U,n,m+1);

% Interpolate V values ( initial conditions )
Vgiven = cos(Y);
V = NtoV * Vgiven(:);
V = reshape(V,n+1,m);

% Actual solution, derivative of sin + cos = cos - sin
Cgiven = cos(X)-sin(Y); 
C = NtoC * Cgiven(:);
C = reshape(C,n+2,m+2);
...
% All we are doing is the divergence of U
