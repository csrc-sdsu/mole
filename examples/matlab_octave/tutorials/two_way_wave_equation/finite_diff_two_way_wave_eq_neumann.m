function [ U2_fd, error_fd, walltime_fd, flops_fd ] = finite_diff_two_way_wave_eq_neumann(c, dt, num_cells)
...
for cell_index = 1 : numel(num_cells)
    ...
    % Build sparse matrix consisting of diagonals A and B
    LTCS = spdiags([B A B], [-1,0,1], nx, nx);

    % Boundary Conditions
    LTCS(1,1) = 1;     LTCS(1,2) = -1;
    LTCS(end,end) = 1; LTCS(end,end-1) = -1;

    U1_fd = 0.5 * LTCS * U0_fd;

    U1_fd(1)   = U1_fd(2);
    U1_fd(end) = U1_fd(end-1);
