...
int main() {   
  ...
  vec psi_old = vectorise(A * sin(nx * M_PI / Lxy * X) % sin(ny * M_PI / Lxy * Y)); // Adjust the sign of the wavefunction   
  ...
  // Apply the interpolation to update psi_old   
  vec psi_interp = I2 * psi_old; // Apply the interpolator to psi_old   
  ...
  // Print the final time step output to the terminal   
  std::cout << "Time Step: " << p << std::endl;   
  ...
  std::cout << "--- End of Time Step " << p << " ---" << std::endl;   
