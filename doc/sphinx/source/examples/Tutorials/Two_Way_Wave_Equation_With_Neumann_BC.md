# Mimetic Difference Operators Vs Finite Differences with Neumann Boundary Condition
### Solving the 1D Two-Way Wave Equation

This tutorial demonstrates how to solve the **1D two-way wave equation** using both **standard finite differences (FD)** and **mimetic finite differences (MD)** with Neumann Boundary Conditions.  
We will use three main functions:

1. [`finite_diff_two_way_wave_eq_neumann`](#finite-diff-two-way-wave-eq)
2. [`mimetic_diff_two_way_wave_eq_neumann`](#mimetic-diff-two-way-wave-eq)
3. [`comparison_two_way_wave_md_vs_fd_neumann`](#comparison-two-way-wave-md-vs-fd)

The goal is to understand the **differences in accuracy, stability, and numerical properties** when there is a boundary condition between traditional finite and mimetic difference operators. The two numerical functions have been created to be as similar as possible, so that only the changes necessary for mimetic difference operators are visible.


The easiest way to compare the two methods is with `comparison_two_way_wave_md_vs_fd_neumann`
...
We assume a Neumann boundary condition where $\frac{du}{dx}(-2,t)=0$ and $\frac{du}{dx}(2,t)=0$.

---
## `finite diff two way wave eq with neumann boundary condition`

**Purpose:** This function solves the wave equation using **standard finite differences (FD)**.
...
The boundary conditions will be added to the Leapfrog Time Centered Scheme matrix.
...
### Notes
We are using an explicit two-step scheme (leapfrog). The CFL condition must be satisfied: $ c \Delta t^2 / \Delta x^2 \le 1 $. If you change the stepping, or increase the number of cells, be sure to change this value. 
...
$$
(I-(\frac{c^2\Delta t^2}{2})L)U^{k+1}_i = 2U^k_i - (I-(\frac{c^2\Delta t^2}{2})L)U^{k-1}_i
$$

Here, we can set A = $I-(\frac{c^2\Delta t^2}{2})L$ and b = $2U^k_i - (I-(\frac{c^2\Delta t^2}{2})L)U^{k-1}_i$, which rearranges our equation to:
$$A U^{k+1}_i = b$$

Now we can add our boundary condition with addScalarBC1D() and solve for $U^{i+1}$.
