...
def analyze_octave_code(matlab_src_dir):
    """
    ...
    Args:
        matlab_src_dir: Directory containing Octave source files
    """
    ...
    print(f"Analyzing Octave code in {matlab_src_dir}")
    ...
    octave_files = glob.glob(os.path.join(matlab_src_dir, "*.m"))
...
def get_function_description(func_name, matlab_src_dir):
    ...
    Args:
        ...
        matlab_src_dir: The directory containing Octave source files
...
def m2html_style_formatter(app, what, name, obj, options, lines):
    ...
    if matlab_src_dir and not _analyzed_code:
        analyze_octave_code(matlab_src_dir)
