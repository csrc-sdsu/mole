...
def analyze_octave_code(octave_src_dir):
    """
    Analyze Octave code to build a call graph.
    
    This function parses all Octave files in the source directory to identify
    ...
    print(f"Analyzing GNU Octave code in {octave_src_dir}")
...
def get_function_description(func_name, octave_src_dir):
    """
    Get the PURPOSE block description of a Octave function.
...
def m2html_style_formatter(app, what, name, obj, options, lines):
    ...
    if m2html_style and len(lines) > 0:
        ...
        new_lines.append(".. code-block:: octave")
        new_lines.append("")
