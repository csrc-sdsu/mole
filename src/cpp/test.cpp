...
// It's appropriate to define it here as a `static` function. Since it's:
//
// 1. **Only used internally** in this `.cpp` file (for the 2-D and 3-D constructors)
// 2. **File-scoped** with `static` linkage (not accessible from other translation units)
// 3. **A simple helper** with no need for external visibility
//
// Defining it in the `.cpp` file keeps the implementation detail encapsulated and avoids polluting the header. You would only move it to the header if it needed to be used elsewhere or if it were part of the public API.
//
Gradient::Gradient(u16 k, u32 m, u32 n, Real dx, Real dy,
