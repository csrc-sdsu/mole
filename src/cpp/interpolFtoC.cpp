...
// 1-D Constructor
InterpolFtoC::InterpolFtoC(u16 k, u32 m, const ivec& dc, const ivec& nc)
{
    assert(dc.n_elem == 2);
    assert(nc.n_elem == 2);
    ...
    {
        // Periodic case
        InterpolFtoC interpol(k, m, true);
        I = interpol;
    } else {
        // Nonperiodic case
        InterpolFtoC interpol(k, m);
        I = interpol;
    }
...
// 2-D Constructor
InterpolFtoC::InterpolFtoC(u16 k, u32 m, u32 n, const ivec& dc, const ivec& nc)
{
    assert(dc.n_elem == 4);
    assert(nc.n_elem == 4);
...
// 3-D Constructor
InterpolFtoC::InterpolFtoC(u16 k, u32 m, u32 n, u32 o, const ivec& dc, const ivec& nc)
{
    assert(dc.n_elem == 6);
    assert(nc.n_elem == 6);
...
InterpolFtoC::InterpolFtoC(u16 k, u32 m) : sp_mat(m + 2, m + 1)
{
    assert(!(k % 2));
    assert(k > 1 && k < 9);
    assert(m > 2 * k);
...
InterpolFtoC::InterpolFtoC(u16 k, u32 m, bool dummy) : sp_mat(m, m)
{
    assert(!(k % 2));
    assert(k > 1 && k < 9);
    assert(m > 2 * k);
