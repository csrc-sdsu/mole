...
% ----------------------------------------------------------------------------
    ...
    switch grid.dim
    case 1
        dx = 1; if isfield(grid, 'dx'); dx = grid.dx; end
        [A, b] = addScalarBC1D_impl(A, b, k, grid.m, dx, ...
                                    grid.bc.dc, grid.bc.nc, v);
    case 2
        dx = 1; if isfield(grid, 'dx'); dx = grid.dx; end
        dy = 1; if isfield(grid, 'dy'); dy = grid.dy; end
        [A, b] = addScalarBC2D_impl(A, b, k, grid.m, dx, ...
                                    grid.n, dy, ...
                                    grid.bc.dc, grid.bc.nc, v);
    case 3
        dx = 1; if isfield(grid, 'dx'); dx = grid.dx; end
        dy = 1; if isfield(grid, 'dy'); dy = grid.dy; end
        dz = 1; if isfield(grid, 'dz'); dz = grid.dz; end
        [A, b] = addScalarBC3D_impl(A, b, k, grid.m, dx, ...
                                    grid.n, dy, ...
                                    grid.o, dz, ...
                                    grid.bc.dc, grid.bc.nc, v);
    otherwise
