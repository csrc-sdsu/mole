...
gridRaw parse_va_args(std::stack<MOLE_Errors>& errs, 
    ...
    for (const char* name = firstName; name != nullptr;
        ...
        // isPeriodic is passed as a pointer to bool[dim]. its 
        // dimention may not be parsed yet given the variable order
        // of the user input pairs. Thus, a pointer is temporarily 
        // stashed and the flags will be copied into the corresponding
        // structures, once dim is already known.
        else if (std::strcmp(name, "isPeriodic") == 0)
            g.isPeriodicSrc = va_arg(ap, const bool*);
...
// Casts a type-erased array pointer to the concrete flat arrays 
// depending on the dimension implies and copies it into dst. A null
// pointer (attribute not supplied) leaves dst at its default, which
// the MOLE library reads as unset.
template <typename T>
...
gridParams1D narrow1D(const gridRaw& g) {
    ...
    if (g.isPeriodicSrc) p.bc_isPeriodic = g.isPeriodicSrc[0];
    return p;
}
